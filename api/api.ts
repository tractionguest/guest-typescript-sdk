export * from './auditLogs.service';
import { AuditLogsService } from './auditLogs.service';
export * from './batches.service';
import { BatchesService } from './batches.service';
export * from './capacities.service';
import { CapacitiesService } from './capacities.service';
export * from './emailTemplates.service';
import { EmailTemplatesService } from './emailTemplates.service';
export * from './groupVisits.service';
import { GroupVisitsService } from './groupVisits.service';
export * from './hosts.service';
import { HostsService } from './hosts.service';
export * from './invites.service';
import { InvitesService } from './invites.service';
export * from './locations.service';
import { LocationsService } from './locations.service';
export * from './packages.service';
import { PackagesService } from './packages.service';
export * from './registrations.service';
import { RegistrationsService } from './registrations.service';
export * from './signins.service';
import { SigninsService } from './signins.service';
export * from './users.service';
import { UsersService } from './users.service';
export * from './watchlists.service';
import { WatchlistsService } from './watchlists.service';
export const APIS = [AuditLogsService, BatchesService, CapacitiesService, EmailTemplatesService, GroupVisitsService, HostsService, InvitesService, LocationsService, PackagesService, RegistrationsService, SigninsService, UsersService, WatchlistsService];
